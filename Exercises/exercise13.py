"""Exercise 13
    ...
    gives us patches in the various pictures which contain pollen.  We
    also need negative samples.  To obtain those, we are going to pick
    5 random 96 pixel by 96 pixel regions in each picture and check
    whether they overlap any of the regions marked as containing
        ...
        with open(full_label_file) as f:
            ...
            for _ in range(neg_per_image):
                ...
                if not any_overlap(pollen, random_box):
                    training_data.append((image_file, (class_id, ???, ???, ???, ???)))  # Fix ME!
...
if __name__ == "__main__":
    ...
    training_data = generate_labelled_data(???, ???, ???)    # Fix ME!
